...
import { Zap, Loader2, X, Trophy } from 'lucide-react';
// 🌟 Use the relative path to your config
...
const EducationPage = () => {
  ...
  const handleClaimMasterclass = async () => {
    try {
        ...
        console.error("Masterclass claim failed", err);
    }
  };
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="relative rounded-3xl bg-gradient-to-r from-dark-800 to-dark-900 p-8 md:p-12 border border-dark-700 shadow-xl overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-gray-400 mb-8 max-w-2xl">Learn the industrial process of extracting gold and silver from high-end motherboards while staying safe.</p>
          <button 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in zoom-in-95">
          <div className="bg-dark-900 border border-dark-700 w-full max-w-4xl rounded-3xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto space-y-8">
              <div className="aspect-video rounded-2xl overflow-hidden border-4 border-dark-700 bg-black">
                <iframe width="100%" height="100%" src="https://www.youtube.com/embed/nvNbDV1Yu-Q?rel=0" title="Masterclass Video" frameBorder="0" allowFullScreen></iframe>
              </div>
              <div className="bg-dark-800 p-6 rounded-2xl border border-dark-700">
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <button onClick={() => alert("Try again!")} className="text-left p-3 rounded-xl border border-dark-700 bg-dark-900 text-gray-400">It's too loud.</button>
                </div>
              </div>
              <button onClick={handleClaimMasterclass} disabled={masterclassQuizStep === 0} className={`w-full py-4 rounded-2xl font-bold text-lg transition-all ${masterclassQuizStep === 1 ? 'bg-waste-500 text-dark-900' : 'bg-dark-700 text-gray-500 cursor-not-allowed'}`}>
                Claim 500 Tokens
              </button>
            </div>
          </div>
